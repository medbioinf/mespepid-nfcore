# nf-core/mspepid: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/mspepid/usage](https://nf-co.re/mspepid/usage)
...
```bash
nextflow run nf-core/mspepid -profile docker -params-file params.yaml
```
...
First, go to the [nf-core/mspepid releases page](https://github.com/nf-core/mspepid/releases) and find the latest pipeline version - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`. Of course, you can switch to another version by changing the number after the `-r` flag.
